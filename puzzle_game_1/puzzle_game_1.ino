...
void titleScreen()
{
  ...
  Point p = ts.getPoint();
  while (!screenTouched) {
    // Loops until the screen is touched
  }
  gameScreen();
}
...
void gameScreen()
{
  Tft.fillScreen(0,238,0,319,BLUE);
  Tft.fillScreen(1,237,1,25,RED);
  Tft.drawString("SLIDING PUZZLE GAME",4,6,2,YELLOW);
  Tft.fillScreen(1,237,26,262,WHITE);
  ...
  Tft.fillScreen(50,189,297,317,CYAN);
  Tft.drawString("NEW GAME",70,300,2,BLACK);
}

boolean screenTouched(Point p)
{
  return (p.z > ts.pressureThreshold);
}
